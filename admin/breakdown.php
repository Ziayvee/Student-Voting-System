...
?>
<table id="resultTable" style="font-size:12px;text-align:left; width: 300px;">
<thead>
	<tr>
		<th> Name </th>
		<th> Course </th>
...
$results = $db->prepare("SELECT * FROM votedetails WHERE candidate= :a");
	...
	for($i=0; $rows = $results->fetch(); $i++){
		$dddddd=$rows['voters'];
		$resultas = $db->prepare("SELECT * FROM students WHERE username= :j");
